  ...
  qCDebug(logEditor) << "MirrorLayerCommand::printMessage(): undo =" << isUndo;
  {
    if ( auto *ms = IMainSystem::instance() ) {
      if ( m_mirrorPlane == 2 ) {
       ...
       IMainSystem::instance()->updateLayerOperationParameter("MirrorLayerCommand::printMessage",QString("Layer %1").arg(m_layerId),LayerItem::OperationMode::Flip,1,0);
      } else if ( m_mirrorPlane == 1 ) {
       IMainSystem::instance()->showMessage(QString("Mirrored layer %1 vertically").arg(m_layerId));
       IMainSystem::instance()->updateLayerOperationParameter("MirrorLayerCommand::printMessage",QString("Layer %1").arg(m_layerId),LayerItem::OperationMode::Flop,1,0);
      } else {
